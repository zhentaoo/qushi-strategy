...
def get_recent_days(days):
    """
    Generate (year, month, day) tuples for the last `days` days.
    """
    # Use UTC as Binance data is in UTC
    # Start from yesterday because today's data might not be ready
    end_date = datetime.now(tz=ZoneInfo('UTC')) - relativedelta(days=1)
            ...
            with zipfile.ZipFile(io.BytesIO(response.content)) as z:
                ...
                try:
                    ...
                    try:
                        ...
                        if "E11000" in str(e): 
                            print(f"  Data already exists in DB for {symbol} {date_str}")
                        else:
